...
% MA-RBF Euler's methods without the condition C2 and with C2 for 
...
err_MAeulerC2_1 = zeros(size(N));
err_MAeulerC2_2 = zeros(size(N));
err_MAeulerC2_3 = zeros(size(N));
...
ord_MAeulerC2_1 = [NaN, log2(err_MAeulerC2_1(1:end-1)./err_MAeulerC2_1(2:end))];
ord_MAeulerC2_2 = [NaN, log2(err_MAeulerC2_2(1:end-1)./err_MAeulerC2_2(2:end))];
ord_MAeulerC2_3 = [NaN, log2(err_MAeulerC2_3(1:end-1)./err_MAeulerC2_3(2:end))];

%% Table generation
met_name = {'Euler', 'MA-RBF Euler 2', 'MA-RBF Euler 2 $p=1$, $L=0$', 'MA-RBF Euler 2 $p=1$, $L=h^{-1}$',...
           'MA-RBF Euler 2 $p=\frac{1}{2}$, $L=h^{\frac{1}{2}}$'};
met_err = {err_euler, err_MAeuler, err_MAeulerC2_1, err_MAeulerC2_2, err_MAeulerC2_3};
met_ord = {ord_euler, ord_MAeuler, ord_MAeulerC2_1, ord_MAeulerC2_2, ord_MAeulerC2_3};
...
g_MAeuler = loglog(N, err_MAeuler, 'm-s', 'LineWidth', 1, 'MarkerSize', 7);
g_MAeulerC2_1 = loglog(N, err_MAeulerC2_1, 'k-*', 'LineWidth', 1, 'MarkerSize', 7);
g_MAeulerC2_2 = loglog(N, err_MAeulerC2_2, 'g-d', 'LineWidth', 1, 'MarkerSize', 7);
g_MAeulerC2_3 = loglog(N, err_MAeulerC2_3, 'r-o', 'LineWidth', 1, 'MarkerSize', 7);
legend([g_euler, g_MAeuler, g_MAeulerC2_1, g_MAeulerC2_2, g_MAeulerC2_3, g_s1, g_s2], ...
       {'Euler', 'MA-RBF Euler 2', 'MA-RBF Euler 2 $p=1$, $L=0$', 'MA-RBF Euler 2 $p=1$, $L=h^{-1}$',...
       'MA-RBF Euler 2 $p=\frac{1}{2}$, $L=h^{-\frac{1}{2}}$', 'Slope -1', 'Slope -2'}, ...
